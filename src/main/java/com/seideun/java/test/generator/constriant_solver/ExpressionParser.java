	...
	public ExpressionNode readNode() {
		...
		while (this.position < this.expression.length()) {
			...
			if (IsLetter(c)) {
				...
				this.position++;
				while (this.position < this.expression.length() && (
					IsLetter(this.expression.charAt(this.position)) ||
						IsDigit(this.expression.charAt(this.position))
				)) {
				...
				if (s.IsWhileSpace(c)) {
					...
					this.position++;
					if (this.expression.length() > this.position &&
						buffer.toString().length() <= 0) {
						continue;
					} else {
						break;
					}
				...
				if ((
					(buffer.length() == 0) || s.IsCongener(
						c,
						buffer.charAt(buffer.length() - 1)
					)
				) && valiateTwo(buffer.toString())) {
					this.position++;
					buffer.append(c);
				} else {
					break;
