...
public class Z3Solver {
	...
	//求解
	public static String solve(String str, List<Object> testValue) throws
		Exception {
		...
		for (String expr: declareBools) {
			exprs.append(expr);
			String temp = expr.replaceAll("\\(declare-const ", "").replaceAll(
				" Real\\)",
				""
			);
			varList.add(ctx.mkRealConst(temp));
		}
		BoolExpr boolExpr = ctx.parseSMTLIB2String(
			exprs.toString() + asserts,
			null,
			null,
			null,
			null
		...
		try {
			...
			if (status.equals("SATISFIABLE")) {
				for (Expr var: varList) {
					...
					testValue.add(s.getModel().eval(var, false));
				}
			} else {
				res.append("");//无解
			}
